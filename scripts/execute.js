...
const FACTORY_NONCE = 1; //Contract nonces start at one. 
...
async function main()
{
    ...
    // This is also needed only in the first run of the ScriptProcessorNode. Aafter that it gets changed to (("0x"))
    const initCode= "0x";//FACTORY_ADDRESS + AccountFactory.interface.encodeFunctionData("createAccount",[address0]).slice(2);

    console.log({sender});

    //This is needed only in the first run of this script.
